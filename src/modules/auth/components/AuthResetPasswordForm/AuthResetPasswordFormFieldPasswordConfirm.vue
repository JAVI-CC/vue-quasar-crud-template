...
import { messageRequired, messageMaxLength, messageMinLength, messageSameAsPassword, messageIncorrectFormat } from 'src/modules/common/helpers/messageRules';
...
const { password, password_confirmation } = toRefs(inject(AuthChangePasswordFormType) as AuthChangePasswordForm);
const isShowPwd = ref<boolean>(false);
</script>

<template>
  <q-input class=" q-mb-md" dense outlined v-model="password_confirmation" :label="$t('Enter a new password')"
    maxlength="255" :type="isShowPwd ? 'text' : 'password'" reactive-rules :rules="[
      $rules.required(messageRequired),
      $rules.minLength(8, messageMinLength(8)),
      $rules.maxLength(255, messageMaxLength(255)),
      $rules.sameAs(password, messageSameAsPassword),
